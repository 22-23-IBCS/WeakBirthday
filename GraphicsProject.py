...
class Graph:
    ...
    def removeNode(self,win):
        ...
        for node in self.nodes:
            ...
            if dis<30:
                ...
                for edge in self.E:
                    point1x=edge.getP1().getX()
                    point1y=edge.getP1().getY()
                    point2x=edge.getP2().getX()
                    point2y=edge.getP2().getY()
                    cenx=node.getCenter().getX()
                    ceny=node.getCenter().getY()
                    if point1x==cenx and point1y==ceny:
                        edge.undraw()
                        self.E.remove(edge)
                    if point2x==cenx and point2y==ceny:
                        edge.undraw()
                        self.E.remove(edge)
    ...
    def Color(self,win):
        for node in self.nodes:
            colorlist=["white", "blue", "black", "DarkOrange", "red","salmon","yellow","magenta","purple"]
            usedcolor=[]
            for neigh in node.getNeighbors():
                print(neigh.getcolor())
                if neigh.getcolor() not in usedcolor:
                    usedcolor.append(neigh.getcolor())
            for uc in usedcolor:
                    colorlist.remove(uc)
            color=str(colorlist[0])
            print(color)
            node.color(color)
            node.colo=colorlist[0]
...
def main():
    ...
    RemoveNode = Button(win, Point(20, 30), Point(100, 90), "darkgreen", "Remove Node")
    AddEdge = Button(win, Point(120, 530), Point(200, 590), "darkgreen", "Add edge!")
    color1 = Button(win, Point(120, 430), Point(200, 490), "beige", "Color!")
    G = Graph(1, 0, win)
    while True:
        m = win.getMouse()
        if Q.isClicked(m):
